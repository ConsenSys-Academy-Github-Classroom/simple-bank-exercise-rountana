...
contract SimpleBank {
    ...
    function deposit() public payable returns (uint) {
     ...
     require(enrolled[msg.sender]); // check if mapping variable is true
      
      // 3. Add the amount to the user's balance. Hint: the amount can be
      //    accessed from of the global variable `msg`
      // msg.value has the transaction value
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
      ...
      //    sender's balance
      // below is std code. sent has result of transfer. data contains return 
      // value from fallback function
        (bool sent, bytes memory data) = msg.sender.call.value(withdrawAmount)("");
        // if transfer fails, send error message
